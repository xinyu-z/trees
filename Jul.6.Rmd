...
#breathiness:
breathy3d<- scatterplot3d(zAVQI$HNR, zAVQI$CPPS, zAVQI$mzB, xlab = "HNR", ylab = "CPPS", zlab = "Breathiness-Z")
breathy.model2 <- lm(mzB ~ HNR + CPPS, data = zAVQI)
rough3d$plane3d(breathy.model2)
```
...
```{r}
glimpse(zAVQI)
roughVar<-lm(Roughness.x~Roughness.y, data = zAVQI)
summary(roughVar)
```
```{r}
breathyVar<-lm(Breathiness.x~Breathiness.y, data = zAVQI)
summary(breathyVar)
```
