...
AVQI_B <- read.csv("AVQI_B.csv")
AVQI_R <- read.csv("AVQI_R.csv")
...
glimpse(SLP)
```
No need to single out brethiness and roughness for the principal component analysis. Merging the two subjective scores with the AVQI table:
```{r}
library(dplyr)
...
summary(step.model)
```
The best model has one variable: HNR
...
```
Same result. The best model contains one variable: HNR
